...
$$
q_b = a \left(\frac{P}{P_{re}}\right)^b \left(\tau_* - \tau_{cr*}\right)^c
$$

where $q_b$ is non-dimensional bedload flux, $P$ is observed seismic power spectral density (PSD), $P_{re}$ is a reference seismic PSD at a given shear stress, $\tau_*$ is Shield parameter, and $\tau_{cr*}$ is critical Shield required to initiate the movement of sediment particles.

After fitting this above equation with our Pinos field data, we obtained the fitted equation as follows:

$$
q_b = 5.2 \left(\frac{P}{P_{re}}\right)^{0.25} \left(\tau_* - \tau_{cr*}\right)^{1.36}
$$
